...
function createDependencyFileJS(allResults){
    ...
    allResults.forEach(function callback(promise, pr, prArray) {
        console.log("Da Promise:");
        console.log(JSON.stringify(promise));
        if(promise.reason && promise.reason.success && !promise.value)//If individual item rejected, but it was a success, just skip
            return;
        else if(!promise.reason && !promise.reason.success) { //If it failed, get outta hear, 'cuz this should never happen.
