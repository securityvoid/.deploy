    ...
    fs.copy(fromDir, toDir,
        function(file){
            for(var i = 0; i < excludeFolders.length; i++){
                ...
                var filter = path.join(process.env.DEPLOYMENT_SOURCE, excludeFolders[i]);
                if(targetFile.startsWith(filter)){
                    return false;
                }
            }
            if(excludeFiles.indexOf(targetFile) >= 0)
                return false;
            return true;
        }, function(err){
            if (err)
...
function checkEnvVariables(){
    ...
    try{
        ...
        process.env.DEPLOY_EXCLUDE_FOLDERS = '[".git", ".deploy", ".idea", "node_modules", "' + process.env.DEPLOY_DIST_FOLDER + '"]';
    }
}
