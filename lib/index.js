...
function generateRequires(pathResults){
    ...
    for (var pattern in paths) {
        ...
        for(var i = 0; i < files.length; i++){
            var replaceValue = files[i].replace(path.normalize(process.env.DEPLOYMENT_SOURCE) + path.sep,'');
            requires += '\tglobal.azureDeps["' + replaceValue +
                '"] =\trequire("' + files[i].replace(/[\\]/g, '\\$&') + '");\n';
            if(path.basename(replaceValue) === 'index.js') {//If index, set alias to the folder as well to catch-more references
