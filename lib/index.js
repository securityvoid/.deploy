...
module.exports.createDistribution = function(webPackLib){
    ...
    fs.emptyDir(path.join(process.env.DEPLOYMENT_SOURCE, process.env.DEPLOY_DIST_FOLDER), function(err){
        if(err)
            deferred.reject({success: false, action: "emptyDir", error: error});
