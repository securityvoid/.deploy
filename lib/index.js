...
function getPaths(file, data){
    ...
    while (matchPathRequire !== null) {
        ...
        paths[wildCardName].replace = (/\.js$/i.test(paths[wildCardName].replace.trim())) ? paths[wildCardName].replace : paths[wildCardName].replace + " + '.js'";
...
function updateIndex(pathResults){
    ...
    fs.writeFile(depFile, pathResults, function(err){
        if(err)
