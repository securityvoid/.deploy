...
function createDependencyFileJS(allResults){
    ...
    allResults.forEach(function callback(promise, pr, prArray) {
        if(promise.reason.success && !promise.value)//If individual item rejected, but it was a success, just skip
            return;
        else if(!promise.reason.success) //If it failed, get outta hear, 'cuz this should never happen.
            deferred.reject({success: false, action: "createDepenencyValidate", fullResults: allResults});
